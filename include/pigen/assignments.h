...
typedef struct { const char *destination; size_t destination_length; const char *expression; size_t expression_length; char destination_code; } pigen_transfer_item;
typedef struct { const char *prefix_end; pigen_transfer_item *items; char **owned_expressions; char *width_lhs; const char *width_rhs; size_t width_rhs_length; size_t count; int requires_exact_width; } pigen_transfer;
...
/* action_code: 0 = invalidate, 1 = flush, 2 = validate, 3 = warned no-op. */
int pigen_extract_clear_action(const char *start, const char *end, pigen_primitives *primitives, const char **prefix_end, const char **target, size_t *target_length, int *action_code);
...
void pigen_emit_width_checks(pigen_string *output, pigen_width_checks *checks, pigen_primitives *primitives);
