	...
	const pigen_rtl_type *descriptor)
{
	...
	if (!model || !descriptor ||
		...
		!pigen_rtl_expr_get(model, descriptor->width_expression)) ||
	...
	for (i = 0; i < model->type_count; i++)
	{
		...
		if (record->signedness == descriptor->signedness &&
			...
			descriptor->width_expression.index)
	...
	size_t child_count)
{
		...
		model->expression_child_capacity)
	{
		model->expression_child_capacity =
			model->expression_child_capacity ?
			model->expression_child_capacity * 2 : 8;
		while (model->expression_child_capacity <
			model->expression_child_count + child_count)
			model->expression_child_capacity *= 2;
	...
	*record = *node;
	...
	if (child_count)
		memcpy(model->expression_children + model->expression_child_count,
			children, child_count * sizeof(*children));
	model->expression_child_count += child_count;
	return result;
}
	...
	pigen_rtl_type_id type, uint64_t value, pigen_source_span origin)
{
	...
	node.kind = PIGEN_RTL_EXPR_INTEGER;
	node.type = type;
	node.origin = origin;
	node.value = value;
