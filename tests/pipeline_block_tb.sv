module pipeline_block_tb;
    ...
    initial begin
        ...
        wait (out_valid);
        if (packet_out !== 16'd23) $fatal(1, "native pipeline result incorrect");
        repeat (2) begin
            @(posedge clk); #1;
            if (!out_valid || packet_out !== 16'd23)
                $fatal(1, "native pipeline did not hold output under backpressure");
        end
        @(negedge clk); out_ready = 1;
        @(posedge clk); @(negedge clk);
        send(16'hffff, 16'd2, 16'd3);
        wait (out_valid);
        if (packet_out !== 16'd1) $fatal(1, "native pipeline truncation result incorrect");
        $display("PASS: native pipeline language block with backpressure");
        $finish;
    end
endmodule
