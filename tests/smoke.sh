...
grep -q 'bram_port__pigen_valid <= ((read_enable)) && 1'"'"'b1;' "$tmp/port_bram.sv"
...
grep -q 'assign stage__pigen_discard = ((discard));' "$tmp/clear_actions.sv"
...
if "$tool" tests/clear_overlap_error.pigen -o "$tmp/clear_overlap_error.sv" 2>"$tmp/clear_overlap_error.log"; then
    echo "expected clear/transfer overlap rejection" >&2
    exit 1
fi
grep -q 'clear action overlaps' "$tmp/clear_overlap_error.log"
