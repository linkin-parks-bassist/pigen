...
"$pigen" tests/degenerate_validate.pigen -o "$temporary/validate.sv" \
	2>"$temporary/validate.err"
test "$(grep -c 'warning: validate on an always-valid wire, reg, or logic transport has no effect' "$temporary/validate.err")" -eq 3
...
iverilog -g2012 -s degenerate_validate -o "$temporary/validate" "$temporary/validate.sv"
...
for kind in reg wire logic; do
	...
	grep -q 'cannot make an always-valid wire, reg, or logic transport invalid' \
		"$temporary/${kind}.err"
done

echo 'PASS: core degenerate-transport diagnostics'
